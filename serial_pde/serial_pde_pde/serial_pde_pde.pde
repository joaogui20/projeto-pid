...
void serialEvent(int serial)
{
  if (serial != NEWLINE) {
    ...
    if(recebido.length>=2){
      ...
      temp[99] = Integer.parseInt(recebido[1]);
      error[99] = Integer.parseInt(recebido[2]);
    }
    ...
    for (int i = 0; i < 99; i++) {
      ...
      stroke(#14FC00);
      line(inicioX, 256 - (temp[i]*2.45), fimX, 256 - temp[i+1]*2.45);
      stroke(#FC0004);
      line(inicioX, 407 - (error[i]*2), fimX , 407 - error[i+1]*2);
    }
    buff = "";
  }
}
